  ...
  S.list()
    ...
    .items([
      ...
      S.listItem()
        ...
        .icon(MdList)
        .child(
          S.editor().id("workList").title("Work list").schemaType("workList").documentId("workList")
        ),
      S.listItem()
        .title("Works")
        .icon(MdFolder)
      ...
      S.listItem()
        ...
        .child(async () => {
          ...
          const works = await client.fetch(
            `*[_type == "work"] | order(title asc) { _id, title, "documentation": documentation[]->._id }`
          )

          // Get all referenced documentation IDs
          const referencedDocIds = new Set(works.flatMap((work: any) => work.documentation || []))
